...
def test_random_pose():
    body_points = example_body_points()
    leg_points = example_leg_points()
    dimensions = example_dimensions()
    poses = example_poses()

    hexapod = VirtualHexapod(dimensions)
    hexapod.update(poses)
    for point_a, point_b in zip(hexapod.body.all_points, body_points):
        assert point_a == point_b

    for leg, leg_set in zip(hexapod.legs, leg_points):
