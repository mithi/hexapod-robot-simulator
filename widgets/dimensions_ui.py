...
sections = [
  make_section_type3(front_input, middle_input, side_input, 'front', 'middle', 'side'),
  make_section_type3(coxia_input, femur_input, tibia_input, 'coxia', 'femur', 'tibia')
]

header = html.Label(dcc.Markdown('**HEXAPOD ROBOT DIMENSIONS**'))
SECTION_DIMENSION_CONTROL = html.Div([
  header,
  html.Div(sections, style={'display': 'flex'}),
  html.Br()
])